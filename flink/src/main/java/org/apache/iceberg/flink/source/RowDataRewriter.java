...
public class RowDataRewriter {
  ...
  public List<DataFile> rewriteDataForTasks(DataStream<CombinedScanTask> dataStream, int parallelism) throws Exception {
    RewriteFunction map = new RewriteFunction(schema, nameMapping, io, caseSensitive,
        encryptionManager, taskWriterFactory);

    CloseableIterator<WriteResult> results = dataStream.map(map).setParallelism(parallelism)
        .executeAndCollect("Rewrite table: " + tableName);

    List<DataFile> dataFiles = Lists.newArrayList();
    while (results.hasNext()) {
      WriteResult result = results.next();
      Preconditions.checkState(result.deleteFiles().length == 0, "Does not support rewrite delete files in format v2.");
      Collections.addAll(dataFiles, result.dataFiles());
    }
