...
public class TestIcebergStreamWriter {
  ...
  public void testWritingTable() throws Exception {
    ...
    try (OneInputStreamOperatorTestHarness<Row, DataFile> testHarness = createIcebergStreamWriter()) {
      ...
      Set<String> dataFilePaths = scanDataFiles();
      Assert.assertEquals(expectedDataFiles, dataFilePaths.size());
      Assert.assertEquals(dataFilePaths, toAbsolutePathSet(testHarness.extractOutputValues()));
  ...
  public void testSnapshotTwice() throws Exception {
    ...
    try (OneInputStreamOperatorTestHarness<Row, DataFile> testHarness = createIcebergStreamWriter()) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        Assert.assertEquals(expectedDataFiles, dataFilePaths.size());
        Assert.assertEquals(dataFilePaths, toAbsolutePathSet(testHarness.extractOutputValues()));
        Assert.assertEquals(dataFilePaths, scanDataFiles());
      }
    }
  }
  ...
  public void testTableWithTargetFileSize() throws Exception {
    ...
    try (OneInputStreamOperatorTestHarness<Row, DataFile> testHarness = createIcebergStreamWriter()) {
      ...
      emittedPaths = toAbsolutePathSet(testHarness.extractOutputValues());
    ...
    Set<String> realDataFiles = scanDataFiles();
    Assert.assertEquals(8, realDataFiles.size());
    Assert.assertEquals(realDataFiles, emittedPaths);
  ...
  private OneInputStreamOperatorTestHarness<Row, DataFile> createIcebergStreamWriter() throws Exception {
    IcebergStreamWriter<Row> streamWriter = IcebergStreamWriter.createStreamWriter(tablePath,
        SimpleDataUtil.FLINK_SCHEMA, CONF);
